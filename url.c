...
{
	return (url != NULL)
		&& ((url->hostText.first != NULL)
			|| (url->hostData.ip6 != NULL)
			|| (url->hostData.ip4 != NULL)
			|| (url->hostData.ipFuture.first != NULL)
			);
}
...
Datum
get_authority(PG_FUNCTION_ARGS)
{
	Datum arg = PG_GETARG_DATUM(0);
	char *s = TextDatumGetCString(arg);
	UriUriA url;
...
get_string(PG_FUNCTION_ARGS)
{
	...
	if (uriToStringCharsRequiredA(&url, &charsRequired) == URI_SUCCESS) {
		elog(INFO, "I need %d", charsRequired);
	}
	charsRequired++;
